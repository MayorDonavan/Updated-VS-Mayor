...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'spookeez' | 'south' | 'monster' | 'vibin' | 'frightin' | 'spookin':
					curStage = 'spooky';
				...
				case 'thorns':
					curStage = 'schoolEvil';
				case 'feesh' | 'yum' | 'feesh-overload' | 'better' | 'beans' | 'miau' | 'found'| 'tuna':
					curStage = 'mayor';
				default:
					curStage = 'mayor';
			}
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					case 'feesh':
						startDialogue(dialogueJson, 'lunchbox');
					case 'yum':
						startDialogue(dialogueJson, 'lunchbox');
					case 'feesh-overload':
						startDialogue(dialogueJson, 'bleh');
					case 'better':
						startDialogue(dialogueJson, 'lunchbox');
					case 'vibin':
						startDialogue(dialogueJson, 'spookycutscene');
					case 'frightin':
						startDialogue(dialogueJson, 'spookycutscene');
					case 'spookin':
						startDialogue(dialogueJson, 'spookycutscene');
